...
static gboolean setSolutionFileIdle(gpointer userData) {
    ...
    g_object_set_data(G_OBJECT(data->board), "solution-queue", NULL);
...
static void fillQueue(Queue* q,FILE* f,int size){
    int sizeSolution=size*2;
...
static void drawAll(cairo_t* cr,Queue* q,int size){
    int* solution=QueuePeekFront(q);
    if(solution){
        double tileScaled=(double)TILE_SIZE*DEFAULT_SIZE/size;
        for(int i=0;i<size;i++){
            int x=solution[2*i];
            int y=solution[2*i+1];
            if(size>=SIZE_COLOUR_LABEL){
                cairo_set_source_rgba(cr,1.0,0.0,0.0,1.0);
                cairo_rectangle(cr,y*tileScaled,x*tileScaled,tileScaled,tileScaled);
                cairo_fill(cr);
            }
            if(imgScaled)
                cairo_set_source_surface(cr,imgScaled,y*tileScaled,x*tileScaled);
            cairo_paint(cr);
        }
    }
    return;
...
static void drawOne(cairo_t* cr,FILE* f,int size){
    double TileSizeScale=(double)TILE_SIZE*DEFAULT_SIZE/(double)(size);
    int i,j;
...
static void onDraw(GtkDrawingArea* area,cairo_t* cr,int width,int height,gpointer userData){
    dataSolve* data=(dataSolve*)userData;
    int allFlag=data->flagAll;

    drawBoard(cr,width,height,&(data->size));
    ...
    if(allFlag==0)
        ...
        Queue* q=g_object_get_data(G_OBJECT(area),"solution-queue");
        if(!q){
            q=InitQueue(size*2);
            fillQueue(q,f,size);    
            g_object_set_data_full(G_OBJECT(area),"solution-queue",q,(GDestroyNotify)QueueFree);
        }
        drawAll(cr,q,size);
    }
    fclose(f);
...
static gboolean ChangeLabel(gpointer userData){
    labelData* lData=(labelData*)userData;
    GtkWidget* label=lData->label;
    uint64_t solves=*(lData->solves);
    gtk_widget_set_opacity(label,1.0);
    if(solves==0)
        gtk_label_set_text(GTK_LABEL(label),"No solutions found");
    else{
...
uint64_t SolNumbersF(char* filename){
    FILE* f=fopen(filename,"rb");
    if(!f)return 0;
...
gpointer SolveThread(gpointer userData) {
    ...
    if (test != NULL) {
        ...
        if(data->flag==1){
            ...
            uint64_t* solves=g_new(uint64_t,1);
            *solves=SolNumbersF(filename);
            GtkWidget* label=g_object_get_data(G_OBJECT(data->button),"label-solutions");
            ...
            uint64_t* dummy=g_new(uint64_t,1);
            *dummy=1;
            lDataNum->solves=dummy;
    ...
    if (data->flag == 1) {
        ...
        *solves=0;
        if (data->fig.inf_movement[HORZ_MOVEMENT]) {
            ...
            *solves=solveAll(f, board, data->fig, data->size, 0, 0);
        }
        labelData* lData=g_new(labelData,1);
        lData->label=label;
        lData->solves=solves;
...
static void GetNewFig(GtkWidget* button,dataAdd* data){
    ...
    for(int i=0;i<SP_MOVEMENT_SZ;i++){
        for(int j=0;j<SP_MOVEMENT_SZ;j++){
            ...
            if(i==CENTER_MOVMENT && j==CENTER_MOVMENT){
...
static gboolean ExitProg(GtkWidget* button,gpointer userData){
    GtkRoot* parent=gtk_widget_get_root(button);
...
static void NextSolution(GtkWidget* button,gpointer userData){ 
    ...
    uint64_t* solNumber=g_new(uint64_t,1);
    *solNumber=q->front->num;
...
static void changeNum(GtkEditable* entry,gpointer userData){
    ...
    if(!hasNonDigit){
        *Number=g_ascii_strtoll(text,NULL,10);
        return;
    }
...
static void goToSolution(GtkWidget* button,gpointer userData){
    labelData* data=(labelData*)userData;
    GtkWidget* board=data->label;
    uint64_t solution=*(data->solves);
    if(solution==0)return;
    Queue* q=g_object_get_data(G_OBJECT(board),"solution-queue");

    if(!q || !q->front)return;
    
    uint64_t current=q->front->num;
    ...
    uint64_t steps;
    if(solution<current){
        steps=current-solution;
        for(uint64_t i=0;i<steps;i++){
            uint64_t curSolve=q->tail->num;
            int* coords=NULL;
            if(QueuePopBack(q,&coords)!=0)return;
            if(QueuePushFront(q,coords)!=0)return;
            if(q->front)q->front->num=curSolve;
        }
    }else{
        steps=solution-current;
        uint64_t curSolve;
        for(uint64_t i=0;i<steps;i++){
            curSolve=q->front->num;
            if(curSolve==1 && i!=0){
                flagCycle=TRUE;
                break;
            }
            int* coords=NULL;
            if(QueuePopFront(q,&coords)!=0)return;
            if(QueuePushBack(q,coords)!=0)return;
            if(q->tail)q->tail->num=curSolve;
        }
        curSolve=q->front->num;
        if(curSolve==1)flagCycle=TRUE;
    }

    if(flagCycle==TRUE){
        uint64_t curSolve=q->tail->num;
        int* coords=NULL;   
        if(QueuePopBack(q,&coords)==0){
            QueuePushFront(q,coords);
            q->front->num=curSolve;
        } 
    }
    ...
    labelData* lData=g_new(labelData,1);
    lData->label=label;
    lData->solves=g_new(uint64_t,1);
    *(lData->solves)=q->front->num;
    ChangeLabelNum(lData);
    gtk_widget_queue_draw(board);
}
...
static void activate(GtkApplication* app, gpointer user_data) {
    ...
    *lastInd=1;
    ...
    gtk_button_set_child(GTK_BUTTON(buttonLeft),imgLeft);
    gtk_widget_set_size_request(buttonLeft,WIDTH/5,-1);
    gtk_widget_set_hexpand(buttonLeft, FALSE);
    gtk_widget_set_halign(buttonLeft, GTK_ALIGN_START);

    uint64_t* dummy=g_new(uint64_t,1);
    *dummy=0;
    uint64_t* Number=dummy;
    ...
    gtk_button_set_child(GTK_BUTTON(buttonRight),imgRight);
    gtk_widget_set_size_request(buttonRight,WIDTH/5,-1);
    gtk_widget_set_hexpand(buttonRight, FALSE);
    gtk_widget_set_halign(buttonRight, GTK_ALIGN_START);

    GtkWidget* boxSaveExit=gtk_box_new(GTK_ORIENTATION_VERTICAL,50);
    ...
    GtkWidget* buttonExit=gtk_button_new_with_label("Exit");
    gtk_widget_set_size_request(buttonExit,WIDTH,-1);
    ...
    gtk_box_append(GTK_BOX(boxQueue),buttonLeft);
